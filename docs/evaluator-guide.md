...
It checks that the public surfaces agree with one another, including the fail-closed payment invariant and the rule that hosted MCP credentials never appear as model-visible tool arguments.
...
The x402 publication state is not treated as equivalent to the web server being reachable.
...
- PyPI Trusted Publishing through GitHub OIDC rather than a long-lived PyPI API token.
